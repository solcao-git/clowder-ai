...
export function buildStaticIdentity(catId: CatId, options?: StaticIdentityOptions): string {
  ...
  lines.push(`${ccName}（operator）。重要决策由${ccName}拍板。需要关注时行首写 ${ccHandles}。`, '');
...
export function buildInvocationContext(context: InvocationContext): string {
  ...
  if (context.pingPongWarning) {
    ...
    lines.push(
      `🏓 乒乓球警告：你和 ${otherLabel} 已连续互相 @ ${context.pingPongWarning.count} 轮。思考是否真的需要再回一棒——第三方介入？收尾给solcao？还是这轮可以不 @？再 @ 2 轮将自动熔断。`,
    );
  }
  ...
  if (context.voiceMode) {
    lines.push(
      'Voice Mode ON: solcao在语音陪伴模式。',
      '- 默认用 audio rich block；代码/表格/长内容用文字并附语音摘要',
      '',
    );
  } else {
    lines.push(
      'Voice Mode OFF: 不强制发语音。默认用文字回复。你仍然可以发 audio rich block，但仅在solcao明确要求语音时才发。',
      '',
    );
  }
  ...
  if (context.mode !== 'parallel' && context.a2aEnabled) {
    ...
    lines.push(
      ...
      `3. 只有solcao本人才能做 → ${cc}（硬条件：不可逆操作 / 愿景级决策 / 跨猫僵局）`,
