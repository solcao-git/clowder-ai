...
describe('F254 queue custody CAS contention (fan-out)', () => {
  test('six concurrent settlements sharing one messageId all commit without exhausting CAS retries', async () => {
    ...
    const seenInvocationIdByCatId = Object.fromEntries(
      allTargetCats.map((catId, index) => [catId, `inv-${index}`]),
    );
    const fanoutEntry = {
      ...entries[0],
    ...
    const settledEntries = entries.map(
      (entry) => queue.getEntrySnapshot(entry.threadId, entry.userId, entry.id),
    );
