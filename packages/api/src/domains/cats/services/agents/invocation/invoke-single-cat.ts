  ...
  try {
    ...
    const processMessage = async (msg: AgentMessage): Promise<AgentMessage[]> => {
      ...
      const processUsageAndContextHealth = async (
        ...
        // dev(tighnari ACP): sessionChain disabled → skip context_health entirely
        // (mirrors pre-merge `if (sessionChainActive)` wrapper around F24 block)
        if (!sessionChainActive) return;
