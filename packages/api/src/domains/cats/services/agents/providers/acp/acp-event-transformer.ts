...
export function transformAcpEvent(
  ...
  switch (sessionUpdate) {
    case 'agent_message_chunk': {
      ...
      // updates that must NOT be emitted as model output text.
      // Only emit when content.type is 'text' or unset (backwards compat).
      if (content?.type && content.type !== 'text') {
        log.debug(
          { catId, contentType: content.type, textLen: content?.text?.length },
          'Suppressing non-text agent_message_chunk',
        );
    ...
    case 'user_message_chunk':
      // ACP protocol internal event — not emitted as output.
      return withFlush(null);
