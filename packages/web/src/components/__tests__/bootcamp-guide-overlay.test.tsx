...
describe('BootcampGuideOverlay', () => {
  ...
  it('keeps the intro full-screen overlay and input tip for phase-1 before any messages', async () => {
    await act(async () => {
      root.render(<BootcampGuideOverlay phase="phase-1-intro" catName="纳西妲" hasMessages={false} />);
    });

    expect(container.textContent).toContain('在下方输入框输入 @纳西妲 你好  开始训练营');
  ...
  it('renders lifecycle tip for phase-7-dev when there are messages', async () => {
    await act(async () => {
      root.render(<BootcampGuideOverlay phase="phase-7-dev" catName="纳西妲" hasMessages />);
    });
  ...
  it('returns null for phase-7.5 (guide engine handles it)', async () => {
    await act(async () => {
      root.render(<BootcampGuideOverlay phase="phase-7.5-add-teammate" catName="纳西妲" hasMessages />);
    });
  ...
  it('returns null for early phases when there are messages', async () => {
    await act(async () => {
      root.render(<BootcampGuideOverlay phase="phase-1-intro" catName="纳西妲" hasMessages />);
    });
