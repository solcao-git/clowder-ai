...
const GEMINI_CAT_CAFE_ENV_PLACEHOLDERS: Readonly<Record<string, string>> = Object.fromEntries(
  MCP_CALLBACK_ENV_KEYS.map((key) => [key, `\${${key}}`]),
);
const KIMI_CAT_CAFE_ENV_PLACEHOLDERS: Readonly<Record<string, string>> = Object.fromEntries(
  MCP_CALLBACK_ENV_KEYS.map((key) => [key, `\${${key}}`]),
);
const QODER_CAT_CAFE_ENV_PLACEHOLDERS: Readonly<Record<string, string>> = Object.fromEntries(
  MCP_CALLBACK_ENV_KEYS.map((key) => [key, `\${${key}}`]),
);
